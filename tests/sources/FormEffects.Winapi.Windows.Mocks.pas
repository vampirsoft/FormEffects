...
{ TWinapiWindowsMocks }
  ...
  private type
    TLPtoDPResultReference = reference to function(const DC: HDC; const Count: Integer): TRect;
  public
    procedure LPtoDP_Result(const LPtoDPResult: TRect); overload;
    procedure LPtoDP_Result(const Callback: TLPtoDPResultReference); overload;

  private type
    TDPtoLPResultReference = reference to function(const DC: HDC; const Count: Integer): TRect;
  public
    procedure DPtoLP_Result(const DPtoLPResult: TRect); overload;
    procedure DPtoLP_Result(const Callback: TDPtoLPResultReference); overload;
  ...
  private type
    TRestoreDCResultReference = reference to procedure(const DC: HDC; const SavedDC: Integer);
  public
...
var
  LPtoDPResultReference: TWinapiWindowsMocks.TLPtoDPResultReference = nil;
...
begin
  PRect(@Points)^ := LPtoDPResultReference(DC, Count);
  Result := True;
end;

procedure TWinapiWindowsMocks.LPtoDP_Result(const LPtoDPResult: TRect);
begin
  LPtoDP_Result(
    function(const DC: HDC; const Count: Integer): TRect
    begin
      Result := LPtoDPResult;
    end
  );
...
procedure TWinapiWindowsMocks.LPtoDP_Result(const Callback: TLPtoDPResultReference);
begin
  if Assigned(Callback) then
    LPtoDPResultReference := Callback
  else
  begin
    LPtoDPResultReference :=
      function(const DC: HDC; const Count: Integer): TRect
      begin
        Result := TRect.Zero;
      end;
...
var
  DPtoLPResultReference: TWinapiWindowsMocks.TDPtoLPResultReference = nil;
...
begin
  PRect(@Points)^ := DPtoLPResultReference(DC, Count);
  Result := True;
end;

procedure TWinapiWindowsMocks.DPtoLP_Result(const DPtoLPResult: TRect);
begin
  DPtoLP_Result(
    function(const DC: HDC; const Count: Integer): TRect
    begin
...
procedure TWinapiWindowsMocks.DPtoLP_Result(const Callback: TDPtoLPResultReference);
begin
  if Assigned(Callback) then
    DPtoLPResultReference := Callback
  else
  begin
    DPtoLPResultReference :=
      function(const DC: HDC; const Count: Integer): TRect
      begin
        Result := TRect.Zero;
      end;
...
type
  TRestoreDC = function(DC: HDC; SavedDC: Integer): BOOL; stdcall;

function RestoreDCMock(DC: HDC; SavedDC: Integer): BOOL; stdcall;
begin
  RestoreDCResultReference(DC, SavedDC);
  Result := True;
end;
...
begin
  ...
  begin
    RestoreDCResultReference :=
      procedure(const DC: HDC; const SavedDC: Integer)
      begin
      end;
  end;
end;
...
{$ENDIF ~ USE_BILLENIUM_EFFECTS}
  ...
  AddIntercept<TLPtoDP>(LPtoDP, LPtoDPMock);
  LPtoDP_Result(nil);

  AddIntercept<TDPtoLP>(DPtoLP, DPtoLPMock);
  DPtoLP_Result(nil);
