...
{$MESSAGE 'Need to test ToolWindowNCPaint'}
procedure ToolWindowNCPaint(const ToolWindow: TToolWindow; const DC: HDC);
{$MESSAGE 'Need to test PaintThemeBorder'}
...
{$MESSAGE 'Need to test WinControlNCPaint'}
...
{$MESSAGE 'Need to test EmulateNCPaint'}
...
{$MESSAGE 'Need to test EraseAndPaintMessage'}
...
{$MESSAGE 'Need to test EmulatePaint'}
...
procedure PaintThemeBorder(const Helper: TRenderWindowToDCHelper; const DC: HDC); overload; inline;
begin
  with Helper do
  begin
  ...
  try
    ...
    begin
      ...
      begin
        ...
        const BevelWidth = FWinControl.GetProtectedBevelWidth;
    ...
    const SavedDCIndex2 = DC.Save;
    try
      NCPrintControl(WinControlWnd, FWinControl, DC);
    finally
      SavedDCIndex2.RestoreDC;
    end;
...
begin
  ...
  begin
    ...
    if IsInheritsClass(FWinControl.ClassType, TToolWindow.ClassName) then
...
procedure EraseAndPaintMessage(const Helper: TRenderWindowToDCHelper; const DC: HDC); overload; inline;
begin
  with Helper do
  begin
    const DoubleBuffered = Assigned(FWinControl) and FWinControl.DoubleBuffered;
    if DoubleBuffered then
      FWinControl.DoubleBuffered := False;

    with DC.Save do
    try
      FWnd.SendMessage(WM_ERASEBKGND, DC, 0);
    finally
      RestoreDC;
    end;
    FWnd.SendMessage(WM_PAINT, DC, FE_ID);

    if DoubleBuffered then
      FWinControl.DoubleBuffered := True;
  end;
end;
...
begin
  const RegControl = TRegControl.Create(Flags, NonClientCallback, ClientCallback);
  const Helper     = TRenderWindowToDCHelper.Create(Wnd, StopWnd, WinControl, RegControl);
  EraseAndPaintMessage(Helper, DC);
end;
{$ENDIF ~ FORM_EFFECTS_TESTS}

procedure EmulatePaint(const Helper: TRenderWindowToDCHelper; const DC: HDC); overload; inline;
begin
  with Helper do
  begin
    ...
    begin
      ...
      (FWinControl as TOleControl).OleDraw(DC);
    end;
  end;
end;
...
begin
  ...
  try
    ...
    try
      DC.SetBrushOrg(BrushOrgPos.X + AbsPos.X, BrushOrgPos.Y + AbsPos.Y);
      PaintClientCallback(
      {$IFDEF FORM_EFFECTS_TESTS}
        FWnd,
        FStopWnd,
