...
begin
  if Wnd.SendMessage(CM_FEFULLRENDER, 0, FE_ID) = FE_ID then
    Exit(RCF_RENDER or RCF_RENDERNC or RCF_FEFULLRENDER);

  Result := 0;
  case Wnd.SendMessage(CM_FENCPAINT, 0, FE_ID) of
    FE_ID - 1: Result := RCF_RENDERNC or RCF_FENCPREPAINT;
    FE_ID    : Result := RCF_RENDERNC or RCF_FENCPAINT;
    FE_ID + 1: Result := RCF_RENDERNC or RCF_FENCPOSTPAINT;
  end;
  case Wnd.SendMessage(CM_FEPAINT, 0, FE_ID) of
    FE_ID - 1: Result := Result or RCF_RENDER or RCF_FEPREPAINT;
    FE_ID    : Result := Result or RCF_RENDER or RCF_FEPAINT;
    FE_ID + 1: Result := Result or RCF_RENDER or RCF_FEPOSTPAINT;
  end;
end;
...
begin
  ...
  if Flags and RCF_FEFULLRENDER <> 0 then
    RegControl.Flags := Flags
  else
  begin
    if Flags and RCF_RENDERNCMASK <> 0 then
    begin
      if Flags and (RCF_FENCPREPAINT or RCF_FENCPOSTPAINT) <> 0 then
      begin
    ...
    if Flags and RCF_RENDERMASK <> 0 then
    begin
      if Flags and (RCF_FEPREPAINT or RCF_FEPOSTPAINT) <> 0 then
      begin
...
begin
  var Result := TRegControl.Create(Flags, NonClientCallback, ClientCallback);
...
//    WinControlType := FWinControl.ClassType;
    ...
    begin // Edge changing
      ...
      MainFormWnd.SetWindowLongPtr(GWL_EXSTYLE, MainFormWnd.GetWindowLongPtr(GWL_EXSTYLE) and not WS_EX_CLIENTEDGE);
...
begin
  ...
  WindowRect.MapWindowRectFromScreen(ToolWindowWnd);
  ClientRect.OffsetRect(-WindowRect.TopLeft);
  DC.ExcludeClipRect(ClientRect);
  ...
  DC.IntersectClipRect(WindowRect);
  DC.FillRect(WindowRect, ToolWindow.Brush.Handle);
end;
...
begin
  ...
  try
    ...
    begin
      ...
      WindowRect.MapWindowRectFromScreen(WinControlWnd);
      ClientRect.OffsetRect(-WindowRect.TopLeft);
      DC.ExcludeClipRect(ClientRect);
      ...
      const WinStyle = WinControlWnd.GetWindowLongPtr(GWL_STYLE);
      ...
      { Erase parts not drawn }
      WindowRect.OffsetRect(-WindowRect.TopLeft);
      DC.FillRect(WindowRect, FWinControl.Brush.Handle);
    end;
...
begin
  ...
  begin
    ...
    if FWinControl is TToolWindow then
      ToolWindowNCPaint(TToolWindow(FWinControl), DC)
    else
    begin
      WinControlNCPaint(
      {$IFDEF FORM_EFFECTS_TESTS}
...
begin
  ...
  begin
    ...
    if HasFlags(RCF_FENCPREPAINT) then
    begin
      with DC.Save do
      try
    ...
    try
      ...
      else if HasFlags(RCF_FENCPAINT) then
        FWnd.SendMessage(CM_FENCPAINT, DC, FE_ID)
      else if HasFlags(RCF_FEFULLRENDER) then
    ...
    if HasFlags(RCF_FENCPOSTPAINT) then
    begin
      with DC.Save do
      try
        FWnd.SendMessage(CM_FENCPAINT, DC, FE_ID)
...
begin
  with Helper do
  begin
    if HasFlags(RCF_FEPREPAINT) then
    begin
      with DC.Save do
      try
    ...
    try
      ...
      else if HasFlags(RCF_FEPAINT) then
        FWnd.SendMessage(CM_FEPAINT, DC, FE_ID)
      else
    ...
    if HasFlags(RCF_FEPOSTPAINT) then
    begin
      with DC.Save do
      try
        FWnd.SendMessage(CM_FEPAINT, DC, FE_ID);
...
begin
  ...
  begin
    ...
    begin
      if ChildWnd.IsWindowVisible then
      begin
        var ChildRect    : TRect;
  ...
  //        ScreenToClient(FWindow, ChildRect.BottomRight);
          ChildRect.MapWindowRectFromScreen(FWnd);
        ...
        begin
          const Point = DC.OffsetWindowOrgEx(-ChildOrgPoint);
...
begin
  ...
  try
    const BrushOrgPos = GetBrushOrgEx;
    try
      DC.SetBrushOrgEx(BrushOrgPos + AbsPos);
      PaintClientCallback(
      {$IFDEF FORM_EFFECTS_TESTS}
        FWnd,
        FStopWnd,
...
begin
  ...
  const WindowRgn  = WindowRect.CreateRectRgn;
  try
    with Helper do
    begin
    ...
    WindowRgn.OffsetRgn(Point);

//    const Region = HRGN.Create(WindowRect);
//    DC.GetClipRgn(Region);
    with DC.CreateClipRgn(WindowRect, False) do
    try
      Region.CombineRgn(WindowRgn, Region);

      const Rect = Region.GetRgnBox;
  ...
  begin
    ...
    begin
      ...
      if FIsMaximizedMDIChild then
        Point := DC.OffsetWindowOrgEx(-FWinControl.Left, -FWinControl.Top);
      try
...
//        DC.GetClipRgn(SaveRgn);
        ...
        try
          const Point = DC.OffsetWindowOrgEx(-ClientOrgPoint);
          try
            DC.IntersectClipRect(ClizetSize);
...
//        DC.GetClipRgn(SaveRgn);
        with DC.CreateClipRgn(False) do
        try
          DC.ExcludeClipRect(ClientOrgPoint, ClizetSize);
...
//      DC.GetClipRgn(SaveRgn);
      with DC.CreateClipRgn(False) do
      try
        DC.IntersectClipRect(ClientOrgPoint, ClizetSize);
        RenderChildWindows(
        {$IFDEF FORM_EFFECTS_TESTS}
          FWnd,
          FStopWnd,
...
begin
  with Helper do
  begin
    if not CheckVisibility or FWnd.IsWindowVisible then
    begin
      LoadWinControlData;
    {$IFDEF USE_TRANSITION_EFFECTS}
    ...
    {$ENDIF ~ USE_TRANSITION_EFFECTS}
      ...
      try
        ...
        begin
          {$MESSAGE WARN 'Not Implemented XP_RENDER and AERO_RENDER'}
          GetRegControl(
            FWnd,
            FWinControl,
          {$IFDEF FORM_EFFECTS_TESTS}
...
begin
  if (Wnd = StopWnd) or (CheckVisibility and not Wnd.IsWindowVisible) then
    Exit;
  ...
  begin
    ...
    Point := DC.OffsetWindowOrgEx(ClientOffset);
  end;
