...
begin
  ...
  TempRect.MapWindow(WinControlWnd, ParentWnd);

  const SaveClipRgn   = TRect.CreateRgn(0, 0, 0, 0);
  const ExistsClipRgn = GetClipRgn(CanvasDC, SaveClipRgn) = 1;
  const ClipRgn       = TempDrawRect.CreateRgn;
  ...
  try
    ...
    try
      ...
      try
        ...
        if not HasUpdateRect then ValidateRect(WinControlWnd, nil);
      end;
    finally
      SetWindowOrgEx(CanvasDC, Point.x, Point.y, nil);
    end;
  finally
    if ExistsClipRgn then SelectClipRgn(CanvasDC, SaveClipRgn) else SelectClipRgn(CanvasDC, 0);
...
begin
  ...
  begin
    ...
    begin
      ...
      if IsScrollBarVisible(FControl, WinControlWnd, sbHorizontal) then
      begin
        const ScrollInfo = TScrollInfo.GetHorzScrollInfo(WinControlWnd, SIF_POS);
        OffsetRect(DrawRect, ScrollInfo.nPos, 0);
      end;
      if IsScrollBarVisible(FControl, WinControlWnd, sbVertical) then
      begin
        const ScrollInfo = TScrollInfo.GetVertScrollInfo(WinControlWnd, SIF_POS);
...
begin
  ...
  begin
    ...
    SetWindowRgn(FBackgoundForm.Handle, TRect.CreateRgn(0, 0, 1, 1), False);
    ...
    begin
      ...
      begin
        ...
        if
          ...
          (TForm(Control).GetInternalFormStyle = TFormStyle.fsMDIChild)
        then
        begin
