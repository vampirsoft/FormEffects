...
uses
  Winapi.Windows,
  System.Rtti, System.Types, System.Classes, System.Generics.Collections,
  Vcl.Controls, Vcl.Forms, Vcl.Graphics,
  FormEffects.TypeHelpers, FormEffects.Utils.ScrollBars
...
uses
  ...
  FormEffects.Utils.Rects, FormEffects.Rendering, FormEffects.Rendering.Pictures;

{$REGION Internal definitions}

procedure DrawXRay(const WinControl: TWinControl; const Canvas: TCanvas; const Rect, DrawRect: TRect; const Size: TSize;
  const PixelFormat: TPixelFormat); inline;
begin
  const WinControlHandle = WinControl.Handle;

  var TempDrawRect := DrawRect;
  LPToDP(Canvas.Handle, TempDrawRect, 2);
    
  var TempRect := Rect;
    
  ClientToScreen(WinControlHandle, TempRect.TopLeft);
  ScreenToClient(WinControl.Parent.Handle, TempRect.TopLeft);
  ClientToScreen(WinControlHandle, TempRect.BottomRight);
  ScreenToClient(WinControl.Parent.Handle, TempRect.BottomRight);
  
  const SaveClipRgn   = CreateRectRgn(0, 0, 0, 0);
  const ExistsClipRgn = GetClipRgn(Canvas.Handle, SaveClipRgn) = 1;
  const ClipRgn       = CreateRectRgn(TempDrawRect.Left, TempDrawRect.Top, TempDrawRect.Right, TempDrawRect.Bottom);
  ...
  try
    ...
    try
      const HasUpdateRect = GetUpdateRect(WinControlHandle, TRect(nil^), False);
      try
        RenderWindowToDC(
          WinControl.Parent.Handle,
          WinControlHandle,
          WinControl.Parent,
          Canvas.Handle,
          TempRect,
        ...
        if not HasUpdateRect then ValidateRect(WinControlHandle, nil);
      end;
    finally
      SetWindowOrgEx(Canvas.Handle, Point.x, Point.y, nil);
    end;
  finally
...
procedure DrawBackgroundForm(const BackgroundOptions: TFEFormContainer.TBackgroundOptions; const Control: TControl;
  const Bitmap: TBitmap; const Rect, DrawRect: TRect; const Size: TSize; const PixelFormat: TPixelFormat); inline;
begin
{$MESSAGE WARN 'Not Implemented DrawBackgroundForm'}
end;

procedure DrawStandardBackground(const Control: TControl; const DC: HDC; const Rect: TRect;
  const ThemesDisabled: Boolean);
begin
{$MESSAGE WARN 'Not Implemented DrawStandardBackground'}
end;

procedure BlendBackground(const BackgroundOptions: TFEFormContainer.TBackgroundOptions; const Bitmap: TBitmap;
  const isLocalBmp: Boolean; const Rect: TRect; const Size: TSize; const PixelFormat: TPixelFormat); inline;
begin
...
begin
  ...
  begin  
    if Assigned(Child.FParent) then Child.FParent.RemoveChild(Child);
...
begin
  ...
  begin  
    ...
    if not ParentBooleanValue then Opaque := Options.Opaque;
...
begin
  ...
  begin
    ...
    begin
      if Child.FParentBackgoundForm then Child.BackgroundFormChange(Self, True);
    end;
  end;
...
begin
  if Assigned(FControl) and (not(csDestroying in Control.ComponentState)) and IsActive then DoChange;

  if Assigned(FParent) then FParent.RemoveChild(Self);

  FreeAndNil(FPicture);

  for var Child in FChildren do RemoveChild(Child);
...
begin
  ...
  then
  begin
    if Graphic.Palette = 0 then Graphic.PaletteModified := False
    else
    begin      
...
procedure TFEFormContainer.TBackgroundOptions.DrawBackground(const DC: HDC; const Bitmap: TBitmap;
  const Rect: TRect);
...
begin
  ...
  begin  
    ...
    begin
      const WinControl = FControl as TWinControl;
      if IsScrollBarVisible(FControl, WinControl.Handle, sbHorizontal) then
      begin
        var ScrollInfo := TScrollInfo.Create(SIF_POS);
        GetScrollInfo(WinControl.Handle, SB_HORZ, ScrollInfo);
        OffsetRect(DrawRect, ScrollInfo.nPos, 0);
      end;
      if IsScrollBarVisible(FControl, WinControl.Handle, sbVertical) then
      begin
        var ScrollInfo := TScrollInfo.Create(SIF_POS);
        GetScrollInfo(WinControl.Handle, SB_VERT, ScrollInfo);
      ...
      begin
        ...
        try
          ...
          begin
            TempBitmap.AdjustForTransition(0, LocalRectSize, PixelFormat);
            SetWindowOrgEx(Handle, DrawRect.Left, DrawRect.Top, nil);
          end
          else PixelFormat := TempBitmap.PixelFormat;
          ...
          begin
            ...
            begin
              DrawXRay(WinControl, TempBitmap.Canvas, LocalRect, DrawRect, LocalRectSize, PixelFormat);
            end
            ...
            else DrawStandardBackground(FControl, Handle, DrawRect, FThemesDisabled);
            ...
            begin
              DrawPicture(
                ...
                TWinControl(ParentForPicture.FControl),
                FControl,
          ...
          if IsDestroyBitmap then BitBlt(DC, Handle, LocalRect, DrawRect, cmSrcCopy);
        finally
          Unlock;
          if IsDestroyBitmap then FreeAndNil(TempBitmap);          
        end;
      end;
    end
    else DrawStandardBackground(FControl, DC, LocalRect, FThemesDisabled);
  end
  else DrawStandardBackground(FControl, DC, Rect, FThemesDisabled);
end;

function TFEFormContainer.TBackgroundOptions.FindParentWithBackgroundForm: TBackgroundOptions;
begin
  if FParentBackgoundForm and Assigned(FParent) then Exit(FParent.FindParentWithBackgroundForm);
  Result := Self;
end;

function TFEFormContainer.TBackgroundOptions.FindParentWithGlass: TBackgroundOptions;
begin
  if FParentGlass and Assigned(FParent) then Exit(FParent.FindParentWithGlass);
  Result := Self;
end;
...
begin
  if FParentPicture and Assigned(FParent) then Exit(FParent.FindParentWithPicture);
  Result := Self;
end;
...
begin
  if Assigned(FPicture.Graphic) then Exit(FPicture.Graphic.Palette);
  Result := 0;
end;
...
begin
  ...
  begin
    for var Child in FChildren do
    begin
      if Child.FParentGlass then Child.GlassChange(Self, True);
    end;
  end;
...
begin
  if Assigned(FControl) then Exit(TActive.Create(IsOpaqueActive, IsBackgroundFormActive, IsPictureActive, IsGlassActive));
...
begin
  for var Child in FChildren do
  begin
    if Child.FParentOpaque then Child.OpaqueChange(Self);
  end;
...
begin
  ...
  begin
    for var Child in FChildren do
    begin
      if Child.FParentPicture then Child.PictureChange(Self, True);
    end;
  end;
...
begin
  ...
  begin  
    ...
    SetWindowRgn(FBackgoundForm.Handle, CreateRectRgn(0, 0, 1, 1), False);
...
procedure TFEFormContainer.TBackgroundOptions.SetControl(const Value: TControl);
{$MESSAGE WARN 'Not Implemented TFEFormContainer.TBackgroundOptions.SetControl'}
  ...
  begin
    ...
    begin
      ...
      begin      
        ...
        if Assigned(PropertyType) and (PropertyType.ToString <> TBackgroundOptions.ClassName) then OptionsProperty := nil;
      end;
    end;
    ...
    begin    
      ...
      if not PropertyValue.TryAsType<TBackgroundOptions>(Result) then Result := nil;          
    end
    else Result := TBackgroundOptions(Control.Perform(CM_FEGETBACKGRONDOPTIONS, 0, 0));
  end;
  ...
  begin
    ...
    begin    
      if Assigned(Control.Parent) then ParentControl := Control.Parent
      else
      begin
        if Control is TWinControl then ParentControl := FindControl((Control as TWinControl).ParentWindow);
        if (ParentControl = nil) and (Control is TForm) and (TForm(Control).FormStyle = fsMDIChild) then
        begin        
    ...
    begin
      ...
      if Result = nil then Result := GetParentBackgroundOptions(RttiContext, ParentControl);        
    end
    else Result := nil;
  end;
  ...
  begin
    ...
    begin      
      ...
      if Assigned(Options) then AddChild(Options)
      else if Child is TWinControl then CheckChildBackgroundOptions(RttiContext, Child as TWinControl);
    end;
  end;
...
begin
  ...
  begin
    ...
    try
      ...
      if Assigned(ParentBackgroundOptions) then ParentBackgroundOptions.AddChild(Self);
      ...
      begin
        ...
        CheckChildBackgroundOptions(RttiContext, TWinControl(FControl));
      end;    
    finally
      RttiContext.Free;
    end;
...
begin
  ...
  begin
    if Value and Assigned(FBackgoundForm) then FreeAndNil(FBackgoundForm);
...
begin
  ...
  begin
    if Value then FPicture.Graphic := nil;
...
constructor TFEFormContainer.TBackgroundOptions.TActive.Create(const OpaqueActive, BackgroundFormActive, PictureActive,
  GlassActive: Boolean);
begin
