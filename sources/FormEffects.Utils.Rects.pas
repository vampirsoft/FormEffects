...
begin
  ...
  if (GraphicSize.Width = 0) or (GraphicSize.Height = 0) then Exit(TRect.InlineCreate(0, 0, 0, 0));

  with TPictureRectHelper.Create(WinControl, Control) do
  begin
    GetClientRect(WinControlHandle, DrawRect);
    ...
    begin
      var ScrollInfo := TScrollInfo.Create(SIF_ALL);
      GetScrollInfo(WinControlHandle, SB_HORZ, ScrollInfo);
      HorizontalOffset := ScrollInfo.nPos;
    end;
    if IsScrollBarVisible(ResolveWinControl, WinControlHandle, sbVertical) then
    begin
      var ScrollInfo := TScrollInfo.Create(SIF_ALL);
      GetScrollInfo(WinControlHandle, SB_VERT, ScrollInfo);
      VerticalOffset := ScrollInfo.nPos;
    end;

    DrawRect := TRect.InlineCreate(0, 0, ClientSize);
...
//      ScreenToClient(ControlHandle,    Result.BottomRight);
      ...
      begin
        ...
        GetScrollInfo(ControlHandle, SB_HORZ, ScrollInfo);
      ...
      begin
        var ScrollInfo := TScrollInfo.Create(SIF_POS);
        GetScrollInfo(ControlHandle, SB_VERT, ScrollInfo);
