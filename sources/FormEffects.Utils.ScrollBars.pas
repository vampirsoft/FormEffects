...
begin
  if Control is TScrollingWinControl then Exit((Control as TScrollingWinControl).GetControlScrollBar(Kind));
  Result := nil;
end;
...
begin
  if Self = sbVertical then Exit(WS_VSCROLL);
  Result := WS_HSCROLL;
end;

function TScrollBarKindHelper.GetBarType: Integer;
begin
  if Self = sbVertical then Exit(SB_VERT);
  Result := SB_HORZ;
end;
...
begin
  if Kind = sbVertical then Exit(Self.VertScrollBar);
...
begin
  ...
  Result :=
    ((ControlScrollBar = nil) or ControlScrollBar.Visible) and
    (GetWindowLongPtr(Wnd, GWL_STYLE) and Style <> 0);

  if Result then
  begin
    const BarFlag   = Kind.GetBarType;
    var ScrollInfo := TScrollInfo.Create(SIF_RANGE);
    GetScrollInfo(Wnd, BarFlag, ScrollInfo);
