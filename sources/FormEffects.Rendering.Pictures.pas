...
procedure DrawPicture(
  ...
  const Rect: TRect;
  const Margin: Word
); overload;{$IFNDEF FORM_EFFECTS_TESTS}inline;{$ENDIF}
procedure DrawPicture(
  const Graphic: TGraphic;
  const PictureMode: TBackgroundPictureMode;
  const TransparentColor: TColor;
...
{ TDrawPictureHelper }
  ...
  public
    class function Create(
      const DC: HDC;
      const UseClipRgn: Boolean;
      const Rect: TRect
    ): TDrawPictureHelper; inline; static;

  public
    procedure Delete; inline;
    procedure IntersectClipRect; inline;
  end;
...
class function TDrawPictureHelper.Create(
  const DC: HDC;
  const UseClipRgn: Boolean;
  const Rect: TRect
): TDrawPictureHelper;
...
procedure TDrawPictureHelper.IntersectClipRect;
begin
  if FUseClipRgn then
    FDC.IntersectClipRect(FRect);
end;
...
begin
  ...
  begin
    ...
    if Graphic is TBitmap then
      TBitmap(Graphic).TransparentColor := TransparentColor;
  end;
  ...
  try
    ...
    case PictureMode of
      ...
      TBackgroundPictureMode.TopRight,
      TBackgroundPictureMode.BottomLeft,
      TBackgroundPictureMode.BottomRight: Canvas.Draw(GraphicRect.Left, GraphicRect.Top, Graphic);

      TBackgroundPictureMode.Stretch,
      TBackgroundPictureMode.CenterStretch: Canvas.StretchDraw(GraphicRect, Graphic);

      TBackgroundPictureMode.Tile: Canvas.Tile(Rect, GraphicRect, Graphic);

      TBackgroundPictureMode.Zoom: Canvas.Zoom(GraphicRect, Graphic);
    end;
  finally
    Delete;
  end;
