...
procedure GetClientSize(WinControl: TWinControl; Window: HWnd; IsMaximizedMDIClient, IsMaximizedMDIChild: Boolean;
  var ClientWidth, ClientHeight: Integer; var ClientOrg: TPoint); overload;
...
procedure EraseAndPaintMessageExt(
  const Wnd, StopWnd: HWND;
  const WinControl: TWinControl;
  const Flags: DWORD;
  const NonClientCallback, ClientCallback: TTEPaintCallback;
...
procedure PaintThemeBorderExt(
  const Wnd, StopWnd: HWND;
  const WinControl: TWinControl;
  const Flags: DWORD;
  const NonClientCallback, ClientCallback: TTEPaintCallback;
  const DC: HDC;
  const EraseLRCorner: Boolean
);
...
begin
  const RegControl = TTERegControl.Create(Flags, NonClientCallback, ClientCallback);
  try
    EraseAndPaintMessage(DC, WinControl, Wnd);
  finally
    FreeAndNil(RegControl);
  end;
end;
...
{$IFDEF FORM_EFFECTS_TESTS}
procedure PaintNonClient(DC: HDC; WinControl: TWinControl; Window, StopWnd: HWnd;
  TERegControl: TTERegControl);
{$ELSE ~ FORM_EFFECTS_TESTS}
...
{$IFDEF FORM_EFFECTS_TESTS}
procedure PaintClient(DC: HDC; WinControl: TWinControl; Window, StopWnd: HWnd;
  TERegControl: TTERegControl);
{$ELSE ~ FORM_EFFECTS_TESTS}
